...
type TVisibility = "Owner" | "Public" | "Shared" | "None";
...
interface IGeneralMediaSettings {
  ...
  storageSettings?: {
    ...
    cloud?: Array<{
      ...
      baseUrl: string; // Örn: `https://<bucket-name>t.s3.eu-north-1.amazonaws.com/`
...
function getMediaUrl(media: IMediaDTO): string {
...
const generateMediaUrl = (baseUrl: string, filePath: string): string => {
  return `${baseUrl}/${filePath}`;
};

function generateMediaUri(media: IMediaDTO): string {
  return `gp://${media.fileDirectoryName}${media.fileName}`;
}

function generateMediaArn(media: IMediaDTO): string {
...
function getObjectUrlFromArn(arn: string): string {
